  ...
  $.fn.extend({
    letItSnow: function letItSnow(options) {
      ...
      return this.each(function letItSnowEach() {
        var height = $(document).innerHeight()-30;
        ...
        var flake = function flake() {
          ...
          var left = Math.floor(Math.random() * 100000 % ($(this).width()-10 + settings.wind)) - settings.wind;
        ...
        var flakeSpawnDelay = function flakeSpawnDelay(elem, flake, left) {
          ...
          elem.animate({
            ...
            step: function step(now, fx) {
              if (fx.prop === 'left') {
                try {
                  if (now <= 0 || now + settings.size_max+1 >= $(window).innerWidth()-2 || height+settings.size_max+1 >= $(window).innerHeight()) {
                    $(this).hide();
                  } else {
                    $(this).show();
                  }
